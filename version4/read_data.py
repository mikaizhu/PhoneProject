...
class read_data:
    ...
    def data_augmentation(self, x_train, y_train, augmentation_rate=64):
        # 这里只能对训练集进行数据增强
        x, y = [], []
        end = x_train.shape[1] - augmentation_rate
        for idx, data in enumerate(x_train):
            y.extend([y_train[idx]]*augmentation_rate)
            for _ in range(augmentation_rate):
                start = random.choice(range(augmentation_rate))
                x.append(data[start:end+start])
