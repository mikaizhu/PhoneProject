...
class Trainer:
    def __init__(self, **kwargs):
        ...
        self.optimizer_model = kwargs['optimizer_model']
        ...
        with torch.no_grad():
            for feature, label in self.test_loader:
                ...
                for _ in range(self.augmentation_rate):
                    ...
                    aug_feature = feature[:, start:start + self.window_len].reshape(-1, 2, self.args.embed_dim, 60)
                    feat, preds = self.model(aug_feature)
                    loss = self.criterion_xent(preds, label) + self.criterion_cent(feat, label) * 0.001
