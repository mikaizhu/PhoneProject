...
class Trainer:
    def __init__(self, **kwargs):
        ...
        self.criterion_cent = kwargs.get('criterion_cent')
        self.criterion_xent = kwargs.get('criterion_xent')
        self.optimizer_cent = kwargs.get('optimizer_cent')
        self.optimizer_model = kwargs.get('optimizer_model')
    ...
    def train_step(self):
        ...
        for feature, label in self.train_loader:
            ...
            feat, preds = self.model(feature)
            loss_cent = self.criterion_cent(feat, label)
            loss_xent = self.criterion_xent(preds, label)
            #loss = self.criterion(preds, label)
            loss = loss_xent + loss_cent * 0.001 # 0.001 is loss weight
            loss.backward()
    ...
    def test_step(self):
        ...
        with torch.no_grad():
            for feature, label in self.test_loader:
                ...
                label = label.to(self.device)
                feat, preds = self.model(feature)
                self.test_loss += (self.criterion_xent(preds, label) + self.criterion_cent(feat,label) * 0.001) / len(self.test_loader)
