...
class read_data:
    ...
    def read_data_by_round(self, root, epochs=4):
        '''
        小于等于epochs的数据为训练集，一共5个epoch
        '''
        p = Path(root)
        self.phone2id = {}
        label = 0
        x_train, x_test, y_train, y_test = [], [], [], []
        for phone in tqdm(p.iterdir()):
            phone_name = phone.name
            self.phone2id[phone_name] = label
            for epoch in phone.iterdir():
                if int(epoch.name) <= epochs:
                    for dat in epoch.iterdir():
                        dat = str(dat)
                        data = np.fromfile(dat, np.int16)
                        L = len(data) // 8192
                        data = data[:L*8192].reshape(L, 8192)
                        x_train.append(data)
                        y_train.extend([label]*L)
                else:
                    for dat in epoch.iterdir():
                        dat = str(dat)
                        data = np.fromfile(dat, np.int16)
        ...
        self.true_label = list(range(self.num_class))
        return x_train, x_test, y_train, y_test


    def read_data_by_sort_time(self, root='../16ue_20211223/', balance=False):
        ...
        self.trainBincount = np.bincount(y_train)
        self.testBincount = np.bincount(y_test)
        self.num_class = len(self.testBincount)
        self.true_label = list(range(self.num_class))
